...
var ChatroomPage = /** @class */ (function () {
    function ChatroomPage(actionSheetCtrl, fetch, config, route, userService, camera, platform, navCtrl, file, filePath, fireService, utils) {
        var _this = this;
    ...
    ChatroomPage.prototype.sendMsgFire = function (message, type) {
        var msg = {
    ...
    ChatroomPage.prototype.sendMsg = function () {
        ...
        if (this.user_input !== '' || this.user_input_image) {
            if (this.user_input_image) {
                ...
                var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__["HttpHeaders"]().set("Authorization", "Bearer " + this.config.token);
                this.fetch.post('upload-file', payload, headers)
                    .subscribe(function (res) {
                    _this.utils.loadingDismiss();
                    if (res.status) {
                        _this.sendMsgFire(res.url, 'image');
                        _this.user_input_image = null;
                    }
                }, function (err) { return _this.utils.loadingDismiss(); });
            }
            else {
    ...
    ChatroomPage.prototype.getMessages = function () {
        var _this = this;
        this.consult = null;
        this.fireService.getConsult(this.consult_id).subscribe(function (res) {
            if (res.length >= 1) {
                _this.consult_key = res[0].key;
                _this.fireService.getChat(_this.consult_key).subscribe(function (consult) {
                    _this.utils.loadingDismiss();
                    _this.consult = consult;
                    console.log("consult ", _this.consult);
                    _this.msgList = consult.chat ? Object.values(consult.chat) : [];
                    _this.scrollDown();
                });
            }
            else {
                _this.utils.loadingDismiss();
                _this.fireService.createConsult(_this.consult_id);
            }
        }, function (err) {
            _this.utils.toast();
            _this.utils.loadingDismiss();
    ...
    ChatroomPage.prototype.pop = function () {
        this.navCtrl.back();
    };
    ChatroomPage.prototype.getPhoto = function () {
    ...
    ChatroomPage.prototype.TakePhoto = function () {
        var _this = this;
        ...
        this.camera.getPicture(options).then(function (imagePath) {
            _this.convertImage('data:image/jpeg;base64,' + imagePath);
        });
    };
    ChatroomPage.prototype.convertImage = function (photo) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var response, blob;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                switch (_a.label) {
    ...
    ChatroomPage.prototype.uploadfn = function () {
        var _this = this;
        ...
        var trigger = function (event) {
            ...
            if (event.target.files && event.target.files.length > 0) {
                ...
                reader.onload = function () { return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](_this, void 0, void 0, function () {
                    return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                        if (file_1.type == "image/png" || file_1.type == "image/x-png" || file_1.type == "image/jpeg") {
                            console.log(file_1.type);
                            this.user_input_image = file_1;
                            this.sendMsg();
                        }
                        return [2 /*return*/];
                    });
...
var FireService = /** @class */ (function () {
    function FireService(afs, db, configService, fetchService) {
        this.afs = afs;
        this.db = db;
    ...
    FireService.prototype.getConsult = function (consult_id) {
        return this.db.list("consults", function (ref) {
            return ref.orderByChild('consult_id').equalTo(consult_id);
        }).snapshotChanges();
    };
    FireService.prototype.getChat = function (consult_key) {
        return this.db.object("consults/" + consult_key).valueChanges();
    };
    FireService.prototype.sendMsg = function (consult_key, msg) {
        console.log("msg ", msg);
        this.db.list("consults/" + consult_key + "/chat").push(msg);
    };
    FireService.prototype.createConsult = function (consult_id) {
        this.db.list("consults").push({ "status": 1, "user_id": this.configService.user.id, "consult_id": consult_id });
    };
    FireService.prototype.create = function (path, msg) {
        return this.afs.collection(path).add({ "from": 1, type: "text", msg: msg });
    };
    FireService.prototype.changes = function (path) {
        return this.afs.collection(path).snapshotChanges();
    };
    FireService.prototype.get = function (path) {
        return this.afs.doc(path).get();
    };
    FireService.prototype.read = function (path) {
        return this.afs.collection(path).get();
        // return this.afs.collection(path).snapshotChanges();
    };
    FireService.prototype.update = function (path, msg) {
        this.afs.doc(path + '/' + msg).update(msg);
    };
    FireService.prototype.delete = function (path, msg) {
        this.afs.doc(path + '/' + msg).delete();
    };
    FireService.ctorParameters = function () { return [
