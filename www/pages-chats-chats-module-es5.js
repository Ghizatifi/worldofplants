...
var ChatsPage = /** @class */ (function () {
    function ChatsPage(navCtrl, fetch, config, popOverCtrl) {
    ...
    ChatsPage.prototype.showConstractions = function () {
    ...
    ChatsPage.prototype.getChats = function () {
        ...
        this.fetch.get('chats', headers)
            .then(function (res) {
            _this.fetch_requested = true;
            var data;
            console.log("chats s , ", res);
            _this.consults_open = false;
            if (res.status) {
                data = res.data;
            }
            else {
    ...
    ChatsPage.prototype.goforChat = function (chat) {
        console.log("chat id ", chat.id);
        this.navCtrl.navigateForward(['chatroom'], { queryParams: chat });
    };
    ChatsPage.prototype.create_chat = function () {
    ...
    ChatsPage.prototype.createChat = function () {
        var _this = this;
        var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__["HttpHeaders"]().set("Authorization", "Bearer " + this.config.token);
        this.fetch.get('chat/create', headers)
            .then(function (res) {
            if (res.status) {
                var chat = { id: res.consult_id, consults_open: true, status: 1 };
                _this.chatData.push(chat);
                _this.goforChat(chat);
            }
            else {
                _this.fetch.errorToast();
            }
